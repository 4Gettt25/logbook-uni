Javalin Logbook (Maven, Javalin + Hibernate)
...
- Framework: Javalin web framework with Hibernate ORM for data persistence.

Project layout

- App code: `src/main/java`
- Resources: `src/main/resources` (`application.properties`, Flyway migrations, static assets, templates)
- Tests: `src/test/java` and `src/test/resources`

Run

- Prereqs: Java 17, Maven 3.9+.
- Dev (H2 persistent): `mvn exec:java -Dexec.mainClass=com.example.logbook.LogbookApplication`
- Tests: `mvn test`
...
- Default profile uses a file-based H2 database so data persists across restarts.
  ...
  - H2 console: Not available in Javalin version (use database tools directly).
- PostgreSQL profile (`postgres`):
  1. Copy `.env.example` to `.env` and set:
     - `SPRING_PROFILES_ACTIVE=postgres`
     - `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`
  2. `mvn exec:java -Dexec.mainClass=com.example.logbook.LogbookApplication`
  3. Flyway validates/migrates; Hibernate `ddl-auto: update` remains enabled.

Environment (.env)

- `.env` is loaded early via dotenv-java so profile and DB settings are available at bootstrap.
- You can also set OS environment variables instead of `.env`.
- Tests use H2 by default and do not depend on `.env`.
...
- 413 on upload: increase `MAX_FILE_SIZE` / `MAX_REQUEST_SIZE` in `.env` file.
